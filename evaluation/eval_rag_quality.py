...
Usage:
    # Use defaults (sample_dataset.json)
    python eval_rag_quality.py

    # Specify custom dataset
    python eval_rag_quality.py --dataset my_test.json
    python eval_rag_quality.py -d my_test.json

    # Get help
    python eval_rag_quality.py --help
...
class RAGEvaluator:
    ...
    def _display_configuration(self):
        ...
        logger.info("  • Mode:                 Offline (using ground truth, no server connection)")
    ...
    async def generate_rag_response(
        ...
        """
        Generate mock RAG response from ground truth (no server connection).

        Args:
            question: The user query.
            ground_truth: The ground truth answer to use as response.
            client: Deprecated - no longer used.

        Returns:
            Dictionary with 'answer' and 'contexts' keys.
            'contexts' is a list with the ground truth as a single context.
        """
        # Use ground truth as both answer and context (no server connection)
        return {
            "answer": ground_truth,
            "contexts": [ground_truth],  # Ground truth as single context
        }
    ...
    async def evaluate_single_case(
        ...
        position_pool: asyncio.Queue = None,
        pbar_creation_lock: asyncio.Lock = None,
    ) -> Dict[str, Any]:
        """
...
Examples:
  ...
  python lightrag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
        ...
        logger.info("%s", "=" * 70)
        logger.info("🔍 RAGAS Evaluation - Offline Mode (No RAG Server Connection)")
        logger.info("%s", "=" * 70)
